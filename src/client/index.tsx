...
function LoadedSettings({ controller, t }: SettingsInjected) {
  ...
  return (
    <div className="dvt-settings">
      ...
      <section className="dvt-panel"><div className="dvt-panel-title"><h3>{t('autoBridge')}</h3></div><div className="dvt-form-grid">
        <Field label={t('autoBridgeEnabled')}><input type="checkbox" checked={draft.enabled} onChange={(event) => { update('enabled', event.target.checked) }} /></Field>
        <Field label={t('autoBridgeMaxImages')}><Input inputMode="numeric" value={draft.maxImages} onChange={(event) => { update('maxImages', event.target.value) }} /></Field>
      </div></section>
