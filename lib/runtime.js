...
export class VisionToolkitRuntime {
    ...
    async resolveCredentialValue() {
        const resolved = await this.ctx.credentials.resolve(this.config.provider.credential);
        if (resolved === undefined) {
            throw new VisionToolkitError('config', 'the configured credential is not set; set provider.apiKey directly or store it through DSH credentials');
        }
