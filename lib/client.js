...
const en = {
    ...
    autoBridgeEnabled: 'Bridge images to the vision model automatically',
    autoBridgeMaxImages: 'Max images per message',
    limits: 'Limits',
...
const zh = {
    ...
    language: '输出语言',
    autoBridge: '图片自动桥接',
    autoBridgeEnabled: '自动将图片桥接给视觉模型',
    autoBridgeMaxImages: '单条消息最多图片数',
    limits: '限制',
    timeout: '请求超时（毫秒）',
    maxBytes: '最大图片字节数',
    maxPixels: '最大图片像素数',
...
function draftOf(value) {
    return {
        ...
        enabled: value.autoBridge?.enabled ?? true,
        maxImages: String(value.autoBridge?.maxImagesPerMessage ?? 4),
...
function valueOf(draft) {
    return {
        ...
        autoBridge: {
            ...
            maxImagesPerMessage: positiveInteger(draft.maxImages, 'autoBridge.maxImagesPerMessage'),
        },
...
function LoadedSettings({ controller, t }) {
    ...
    const [keySaving, setKeySaving] = (0, react_1.useState)(false);
    const [keySaved, setKeySaved] = (0, react_1.useState)(false);
    const [keyError, setKeyError] = (0, react_1.useState)(undefined);
