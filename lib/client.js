...
function LoadedSettings({ controller, t }) {
    ...
    const save = () => {
        try {
            ...
            void controller.save(key.length === 0 ? value : { ...value, provider: { ...value.provider, apiKey: key } }, snapshot.settings.revision);
        }
        catch (error) {
